window.addEventListener('DOMContentLoaded', () => {
  ...
  console.log(pixelRage.value);

  const screen = {
    width: pixelRage.value,
    height: pixelRage.value,
    pixelSize: 1
  };
  const styles = {
    background: '#000',
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === ' ' || e.code === 'Space') {
      ...
      // 1 gerar os nós do polígono
      const pontos = gerarPoligono();

      // 2 sortear cores
      const corContorno = randomColor();
      const corPreenchimento = randomColor();

      // 3 desenhar/pintar o polígono
      desenharPoligono(pontos, corContorno, corPreenchimento);

      // Atualizar o status
      status.textContent = 'Polígono = ' + pontos.length + ' lados | Contorno: ' + corContorno + ' | Preenchimento: ' + corPreenchimento;
    }
  });
